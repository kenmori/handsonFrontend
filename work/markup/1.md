...
- [mockAPI](https://mockapi.io/)でユーザーのデータを管理する模擬APIを作成します。ダミーのデータをいくつか入れて、ログイン時にこのデータと照合するようにします。動作確認で使用できるようにレビュワーにも伝えてください。
...
```js
{
  userId: "Zella_Homenick38@example.net"
  password: "HQnmjPKBWkqzjeB"
}
...
サーバー内で検証していることを想定するためPromiseを使い、
Promiseのコールバック関数は、mockAPIで取得したデータと、nameとpasswordをそれぞれの値のプリミティブ比較で合っているか検証する内容としてください(※1)。(ここのPromise辺りの実装はなんでもいいです)
合っていたらresolve、違う場合 reject。
（ただし、fetchの際、queryParameterを使ってmockAPIで検証する場合はこの限りではありません。）
（
...
（例）
// 成功時
{ token: "ae2efaa8fd0255cfafda76a7", ok: true, code: 200 }
