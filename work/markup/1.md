...
例えば、右から出てくるように実装してもらいましたが
それを左から出てくるように設定できることを引数に値を渡したら切り替えられるようにしてください
...
そこには検索機があり、入力するとタイトルorタグにマッチした記事が表示され他は非表示になります
...
ページ上部にコンテンツに星マークのアイコンをつけてください
押下するとお気に入りに登録されます。

その記事idと記事タイトル。リンク先。など必要だと思う要素をローカルストレージに保存して(本当はお気に入りのAPIを用意したいですが)

次に同じページを訪れた時(yahoo風のコンテンツAPIを取得する時)には
ローカルストレージにidがあれば星アイコンをdisabled。

マイページに遷移するとお気に入りのリストが作られるようにしてください

※ここの仕様はdraft状態ですので都度わからない場合聞いてください

### 33. ユーザーが見れるコンテンツを分けてください
