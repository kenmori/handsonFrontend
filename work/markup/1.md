...
<!-- コンテンツに星マークのアイコンをつけて
押下するとお気に入りに登録されるページを作ります

まずはyahooニュースの記事にお気に入りのアイコンを追加して(デザインは一旦任意)
そこを押すと、その記事idをローカルストレージに保存して(本当はお気に入りのAPIを用意したいですが)
次に訪れた時(yahoo風のコンテンツAPIを取得する時)にローカルストレージにidがあればそれをリストとして出す
ようにしてください -->


### 31. ユーザーが見れるコンテンツを分けてください
...
それがtrueだったら
ログイン済みなページ
ログイン済みなページでは課題のyahooページのコンテンツを
何かログイン済みユーザー特有のコンテンツに変更してください
この場合ログインボタンは「マイページ」となります
